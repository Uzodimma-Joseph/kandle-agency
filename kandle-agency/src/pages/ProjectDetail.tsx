...
import { getProjectBySlug, getPublishedProjects } from "../services/projectService";

export default function ProjectDetail() {
  const { slug } = useParams();
  const [project, setProject] = useState(() => (slug ? getProjectBySlug(slug) : undefined));

  useEffect(() => {
    setProject(slug ? getProjectBySlug(slug) : undefined);
    window.scrollTo(0, 0);
  }, [slug]);
